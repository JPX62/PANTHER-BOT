...
import Panther.database.notesdb as Panther
...
async def save(client, message):
    ...
    if not arg:
        ...
        return
    note_name = arg
    note = await Panther.get_note(note_name)
    if note:
    ...
    await Panther.save_note(note_name, copy.message_id)
...
async def clear(client, message):
    ...
    if not note:
        ...
        return
    await Panther.rm_note(note_name)
...
async def notes(client, message):
    msg = "**Saved Notes**\n\n"
    all_notes = await Panther.all_notes()
    if not all_notes:

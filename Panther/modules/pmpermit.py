...
from Panther.helpers.pyrohelper import get_arg, denied_users
import Panther.database.pmpermitdb as Pantherdb
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await Pantherdb.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await Pantherdb.set_pm(True)
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await Pantherdb.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Pantherdb.set_permit_message(Pantherdb.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await Pantherdb.set_permit_message(f"`{arg}`")
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Pantherdb.set_block_message(Pantherdb.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Pantherdb.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Pantherdb.get_pm_settings()
    await Pantherdb.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await Pantherdb.deny_user(chat_id)
